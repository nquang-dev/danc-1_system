    ...
    with tab1:
        ...
        st.markdown("""
            ...
            <form action="" method="post">
                <button class="delete-button" type="submit" name="delete_user">Xóa người dùng</button>
            </form>
        """, unsafe_allow_html=True)

        # Xử lý sự kiện nhấn nút
        if st.session_state.get("delete_user_triggered") is None:
            st.session_state.delete_user_triggered = False

        # Kiểm tra nếu form submit (bằng cách xem có param POST)
        import streamlit as st
        import streamlit.components.v1 as components

        # Sử dụng hidden input để đánh dấu khi submit
        components.html(
            """
            <script>
                const form = document.querySelector('form');
                form.addEventListener('submit', () => {
                    window.parent.postMessage({ type: 'delete_user_triggered' }, '*');
                });
            </script>
            """,
            height=0,
        ...
        # Nhận sự kiện từ JS gửi về (dành cho advanced case)
        # Tuy nhiên với phiên bản hiện tại, bạn có thể đơn giản dùng `st.button()` kết hợp CSS với cách dưới đây

        # Đơn giản hơn: dùng Streamlit `st.markdown()` để tạo style cho nút thường:
        col1, col2 = st.columns([3, 1])
        with col1:
            user_to_delete = st.selectbox(
                "Chọn người dùng cần xóa:",
        ...
        with col2:
            # Chèn style riêng cho button bằng cách bao nút trong div
